...
5. **Restore on approval:** ask the user why they're rolling back (shared rule 4), then run `restore --backup <zip> --name <skills…> --reason "<short label>" --rationale "<their reason>"`. It takes a safety backup first, replaces the folders, logs a "restored" event and refreshes those skills' baseline.
